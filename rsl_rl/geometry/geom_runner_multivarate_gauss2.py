...
class GeometryRunnerMVN2:
    def __init__(self, env: VecEnv, device: torch.device, steps_per_it, min_it, policy_it, goem_it):
        ...
        self.min_samples_per_dist = 2 # must be grather than one
    ...
    def sample_geometry(self, envs = None, it = 0): # TODO: reomve
        ...
        if not ((self.number_of_samples_per_dist + envs) <= self.min_samples_per_dist).any() and it > self.min_it:
            ...
            # # TODO: make that no samples are wasted
            # samples = torch.stack([d.sample() for d in self.distribution_log])  
            # print("distribution log size: ", len(self.distribution_log))
        
            # print("sample size: ", samples.size())
            # print("geometry size: ", self.geometry.size())
            # print("geom mask size: ", self.geom_mask)
            # print("envs size: ", envs.size())

            # self.geometry[envs == 1, self.geom_mask == 1] = samples[envs == 1, :]
    ...
    def store_reward(self, reward, it):
        if it > self.min_it:
            # TODO: when first log, set the number of samples per dist to 1
    ...
    def geom_opt_step(self, it):
        ...
        self.rewards = torch.empty((0, self.env.num_envs), device=self.device)
        self.geometry_log = torch.tensor([], device=self.device)
        self.number_of_samples_per_dist = torch.zeros(self.env.num_envs, device=self.device)
        # self.sample_geometry(None, it)
        # reset all envs here? 
        self.env.distribution_update(self.geometry_log)
        self.env.reset()
